...
def test_equals():
    ...
    assert run_source("print(false.equals(1))") == formatted_error(
        "Error at '1': Expected Bool but got Int for parameter other in call to equals.",
        1,
    )


def test_is_true():
    assert run_source("print(true.toBool())") == "true"
    assert run_source("print(false.toBool())") == "false"
