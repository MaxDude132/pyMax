...
def test_addition():
    ...
    assert run_source("print(1 + 'test')") == formatted_error(
        "Cannot add <Int> and <String>", 1
    )
    assert run_source("print(true + true)") == formatted_error(
        "<Bool> does not implement the add method.", 1
    )
    assert run_source("print('true' + true)") == formatted_error(
        "Cannot add <String> and <Bool>", 1
    )
    assert run_source("print(List(1) + List(2))") == "List(1, 2)"
    assert run_source("print(List(1) + 2)") == formatted_error(
        "Can only add a List to a List. Use the push method to add items to a List.", 1
    )
    assert (
...
def test_substraction():
    ...
    assert run_source("print('test' - 't')") == formatted_error(
        "<String> does not implement the substract method.", 1
    )
    assert run_source("print(true - 't')") == formatted_error(
        "<Bool> does not implement the substract method.", 1
    )
    assert run_source("print(List(1, 2) - 1)") == formatted_error(
        "<List> does not implement the substract method.", 1
    )
    assert run_source("print(Map(1 -> 'test') - 1)") == formatted_error(
        "<Map> does not implement the substract method.", 1
    )
    assert run_source("print((1 -> 'test') - 1)") == formatted_error(
        "<Pair> does not implement the substract method.", 1
    )
...
def test_division():
    ...
    assert run_source("print('test' / 't')") == formatted_error(
        "<String> does not implement the divide method.", 1
    )
    assert run_source("print(true / 't')") == formatted_error(
        "<Bool> does not implement the divide method.", 1
    )
    assert run_source("print(List(1, 2) / 1)") == formatted_error(
        "<List> does not implement the divide method.", 1
    )
    assert run_source("print(Map(1 -> 'test') / 1)") == formatted_error(
        "<Map> does not implement the divide method.", 1
    )
    assert run_source("print((1 -> 'test') / 1)") == formatted_error(
        "<Pair> does not implement the divide method.", 1
    )
